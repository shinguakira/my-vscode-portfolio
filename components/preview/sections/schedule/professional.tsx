...
export function ProfessionalSchedule() {
  ...
  const t = locale === "en"
    ? { title: "Schedule", sub: "Availability & Calendar", loading: "Loading calendar...", error: "Unable to load calendar", refresh: "Refresh", open: "Open Full Calendar", tz: "Timezone: Asia/Tokyo (JST)", note: "Feel free to contact me to schedule a meeting or discuss availability." }
    : { title: "スケジュール", sub: "空き状況カレンダー", loading: "カレンダーを読み込み中...", error: "カレンダーを読み込めませんでした", refresh: "更新", open: "フルカレンダーを開く", tz: "タイムゾーン: Asia/Tokyo (JST)", note: "ミーティングのスケジュールについてはお気軽にお問い合わせください。" }

  const calendarUrl = `https://calendar.google.com/calendar/embed?src=${encodeURIComponent(CALENDAR_ID)}&ctz=Asia%2FTokyo`

  return (
    <div className="min-h-full bg-white">
      <div className="max-w-5xl mx-auto px-4 short:px-2 py-24 short:py-8">
            ...
            {error ? (
              <div className="flex items-center justify-center h-96 bg-gray-50">
                <div className="text-center">
                  ...
                  <button onClick={() => window.location.reload()} className="px-4 py-2 border border-gray-300 text-gray-700 hover:bg-gray-50 transition text-sm">
                    {t.refresh}
                  </button>
                </div>
              </div>
